...
class DashboardWindow(QMainWindow):
  ...
  def init_ui(self):
    ...
    self.btn_delete.clicked.connect(self.handle_delete_click)
  ...
  def handle_delete_click(self):
    ...
    if current_row < 0:
      QMessageBox.warning(
        self, "Warning", "Please select a game to delete from the list."
      )
      return
    ...
    if confirm == QMessageBox.StandardButton.Yes:
      success, msg = self.network.delete_game(game_name)
      if success:
        QMessageBox.information(self, "Success", f"Game '{game_name}' deleted.")
        self.refresh_list()
      else:
